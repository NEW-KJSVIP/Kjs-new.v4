...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/NEW-KJSVIP/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/NEW-KJSVIP/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/NEW-KJSVIP/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
wget -q raw.githubusercontent.com/NEW-KJSVIP/Kjs-projeck/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
